...
public class Fetch_Waveforms : BaseNetLogic
{
    ...
    {
        var pm = Owner.Owner.Owner.GetAlias("PM");
        if (pm == null)
        {
            Log.Error("WaveformViewer_List", "PM alias not found");
            return;
        }

        var pmEvent = Owner.Owner.Owner.GetAlias("PMEvent");
        if (pm == null)
        {
            Log.Error("WaveformViewer_List", "PMEvent alias not found");
            return;
        }

        var ipVariable = pm.GetVariable("httpEnable").GetVariable("ipAddress");
        if (ipVariable == null)
        {
            Log.Error("WaveformViewer_List", "ipAddress variable not found in PM tag");
            return;
        }
        ...
        deviceName = pm.GetVariable("nameToDisplay")?.Value;
        if (string.IsNullOrEmpty(deviceName))
        {
            Log.Error("WaveformViewer_List", "nameToDisplay is null or empty");
            return;
        }

        waveformID = pmEvent.GetVariable("WaveformID")?.Value;
        if (string.IsNullOrEmpty(waveformID))
        {
            Log.Error("WaveformViewer_List", "WaveformID is null or empty");
            return;
        }
        ...
        // Check if there are any waveforms associated with the event
        if (pmEvent.GetVariable("WaveformID").Value == "00000000_000000_000000")
        {
            statusLabel.Text = "No waveforms associated with this event.";
            return;
        }

        waveformFilesNode = pm.GetObject("WaveformFiles");
        if (waveformFilesNode == null)
        {
            waveformFilesNode = InformationModel.MakeObject("WaveformFiles");
            pm.Add(waveformFilesNode);
    ...
    public void FetchFileByName(string fileNameToFetch)
    {
        if (fileNameToFetch == null)
        {
            statusLabel.Text = "Error: Filename not found";
            cancelButton.Visible = false;
            return;
        ...
        //Log.Info("WaveformViewer_List", $"Starting new waveform fetch for {fileName}");
        displayWaveformTask = new LongRunningTask(task => FetchAndDisplayWaveform(fileNameToFetch, task), LogicObject);
        displayWaveformTask.Start();
    }
    ...
    private void FetchWaveformList(LongRunningTask task)
    {
        ...
        try
        {
            ...
            var loginManager = new DeviceLoginManager(ipAddress, Owner.Owner.Owner.GetAlias("PM"), deviceName);
            ...
            using (var client = new HttpClient())
            {
                ...
                // Original regex: matches all waveform files in the table
                /*
                ...
                */
                ...
                // New regex: only matches waveform files that contain the waveformID variable value
                var waveformIdPattern = Regex.Escape(waveformID);
                var matches = Regex.Matches(
                    htmlContent,
                    $@"<tr[^>]*><td[^>]*><a[^>]*href=""[^""]*Waveform_\d{{3}}_{waveformIdPattern}_\d{{2}}\.wfm[^""]*"">Waveform_\d{{3}}_{waveformIdPattern}_\d{{2}}\.wfm</a></td>\s*<td[^>]*>(\d+(\.\d+)?)</td>\s*<td[^>]*>(\d+)</td>\s*<td[^>]*>(\d{{1,2}}/\d{{1,2}}/\d{{4}})</td>\s*<td[^>]*>(\d{{1,2}}:\d{{2}}:\d{{2}})</td>",
                    RegexOptions.IgnoreCase);
                ...
                var fileInfos = matches.Cast<Match>()
                    .Select(m => new
                    {
                        FileName = m.Value.Contains("Waveform_") ? Regex.Match(m.Value, $@"Waveform_\d{{3}}_{waveformIdPattern}_\d{{2}}\.wfm").Value : "",
                        Size = float.Parse(m.Groups[1].Value),
                        Cycles = int.Parse(m.Groups[3].Value),
                        DateTime = ParseWaveformDateTimeFromTable(m.Groups[4].Value, m.Groups[5].Value)
                    })
                ...
                FetchFileByName(fileInfos[0].FileName);
